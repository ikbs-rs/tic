...
BEGIN 
    pValue = 0;
RAISE NOTICE 'DOSAO U FUNKCIJU tic_docs_setservices za ID: %', u_id;        
    ...
    AND art IN (
   	...
   	select 	p.*
   	into	cmn_paymenttpRecord
   	from	cmn_paymenttpx_v p 
   	where 	p.id = u_paymenttp;
   
    -- Dodavanje naknade za ONLINE plaćanje Н1
    IF cmn_paymenttpRecord.code = 'OL' THEN
...
--            pTextIznos = pText * 0.01 * pPrice;
           pTextIznos = (pTax * 0.01 * pPrice)/(1 + pTax * 0.01);
            ...
            VALUES (pId, null, tic_docs_record2.doc, tic_docs_record2.event, tic_docs_record2.loc, pArt, pTgp, pTax, pPrice, 0, 
                    ...
                    tic_docs_record2.label, tic_docs_record2.seat, tic_docs_record2.vreme, null, null, 
                    null, 0 );
    ...
    END IF;
   	...
   	RAISE NOTICE 'Dodavanje naknade za NAPLATU NA PRODAJNOM MESTU  Н3: %', u_id;  
   	
   	select *
   	into	channelRecord
   	from	cmn_obj o
   	where 	o.id = u_channel;
   
   	if channelRecord.code != 'WEB' then
		if channelRecord.code = 'WEB' then
		else
...
-- Dodavanje naknade za REZERVACIJU  Н4  
 ...
 RAISE NOTICE 'H4 03 -- %', tic_docs_record4;          
	            INSERT INTO tic_docs 
...
RAISE NOTICE 'H5 -- Delivery: %', u_delivery; 
       	...
       	limit 1;
	        ...
	        loop
		        ...
		        RAISE NOTICE 'H5 -0- tic_docs_record5: %', tic_docs_record5; 
			        -- Za naknadu dovlačim postavke iz eventa - procenat i minimalnu naknadu po karti
			        SELECT s.value::numeric, s.text, s.condition, s.minfee 
			       ...
			       RAISE NOTICE 'H5 -1- tic_docs_record5: %, %', pCountry, pArt;
		            -- Novi ID za tic_docs i tic_docslink
		            pId = nextval('tic_table_id_seq');
		        
		            -- Iznos naknade
		--            pPrice = pIznos;
		        RAISE NOTICE 'H5 -2- tic_docs_record5 Price: %, pTax: %', pPrice, pTax;
		            ...
		            VALUES (pId, null, tic_docs_record5.doc, tic_docs_record5.event, tic_docs_record5.loc, pArt, pTgp, pTax, pPrice, 0, 
		                    ...
		                    tic_docs_record5.label, tic_docs_record5.seat, tic_docs_record5.vreme, null, null, 
		                    null, tic_docs_record5.delivery );
...
RAISE NOTICE 'DOSAO DO KRAJA FUNKCIJE tic_docs_setservices za ID: %', u_id;   
